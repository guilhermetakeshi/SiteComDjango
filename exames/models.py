...
class TiposExames(models.Model):
    # aqui precisamos herdar de models.Model para transformar a classe feita em python, para uma tabela no banco de dados.
    nome = models.CharField(max_length=100)
    tipo_choices = (
        ('I', 'Exame de Imagem'),
        ('E', 'Exame de Eletrocardiograma'),
        ('S', 'Exame de Sangue'),
    )
...
class SolicitacaoExame(models.Model):
    ...
    def badge_template(self):
        if self.status == 'E':
            classes = 'rounded-pill bg-warning text-dark'
            texto = "Em Análise"

        elif self.status == 'F':
            classes = 'rounded-pill bg-success'
            texto = "Finalizado"
